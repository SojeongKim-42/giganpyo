...
class TableViewSets(viewsets.ModelViewSet):
    ...
    def destroy(self, request, user_id, table_id, *args, **kwargs):
        ...
        try:
            ...
            # pass
        if table.main:
            return Response(status=status.HTTP_400_BAD_REQUEST, data={"message": "main테이블은 지울 수 없습니다."})
    ...
    def create(self, request, user_id, *args, **kwargs):
        ...
        if (not table) and (request.data['main']==False):
            return Response(status=status.HTTP_400_BAD_REQUEST, data={"message": "첫 시간표는 main 시간표가 되어야 하므로 main=True로 설정하세요."})
        if (table) and (request.data['main'] == True):
            return Response(status=status.HTTP_400_BAD_REQUEST, data={"message": "이미 main 시간표가 존재하므로 main=False로 설정하세요."})
